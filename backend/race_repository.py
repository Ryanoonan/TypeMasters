...
from sqlalchemy import Sequence, case, func, insert, select, text, update
...
class RaceRepository:
    ...
    def get_public_race_with_most_availability(self) -> ORMRace | None:
        querry = (
            select(ORMRace)
            .join(target=ORMUser, onclause=ORMRace.id == ORMUser.race_id)
            .where(
                ORMRace.visibility == RaceVisibility.PUBLIC,
                ORMRace.state == RaceState.PENDING,
            )
            .group_by(ORMRace.id, ORMRace.max_players)
            .having(func.count(ORMUser.race_id) < ORMRace.max_players)
            .order_by(func.count(ORMUser.race_id))
            .limit(1)
        )
    ...
    def join_race(self, user_id: UUID, race_id: UUID):
        query = (
            update(ORMUser)
            .where(ORMUser.id == user_id, ORMUser.race_id == None)
